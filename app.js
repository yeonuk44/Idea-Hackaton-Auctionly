...
var bodyParser = require('body-parser');       //post 방식 파서
//
...
router.route('/process/community').get(
    function (req, res)
    {
        console.log('/process/community  라우팅 함수 실행');
...
router.route('/process/create_account').post(                      //설정된 쿠키정보를 본다
    function (req, res) {
        ...
        if (req.session.user) {
            ...
            res.write('<a href="/index.html">\t Move</a>');
            ...
            res.write('[ID] : ' + paramID + ' [PW] : ' + pw);
            res.write('<h2>Move to homepage </h2>');
            res.write('<a href="/mypage_login.html">\t Move</a>');
...
appServer.listen(app.get('port'),
    function () {
        console.log('express 웹서버 실행' +'localhost:'+ app.get('port')+'login.html');
    }
);
